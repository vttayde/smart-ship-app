...
export function testVolumetricWeight() {
  ...
  testCases.forEach((testCase, index) => {
    ...
    const billableWeight = pricingCalculator.getBillableWeight(testCase.actualWeight, testCase.dimensions)
    
    console.log(`Test ${index + 1}:`)
    console.log(`  Dimensions: ${testCase.dimensions.length}×${testCase.dimensions.width}×${testCase.dimensions.height} cm`)
    ...
    console.log(`  Expected: ${testCase.expected} kg ${billableWeight === testCase.expected ? '✅' : '❌'}`)
    console.log('')
  })
}
...
export function testZoneMapping() {
  ...
  testRoutes.forEach((route, index) => {
    ...
    console.log(`${index + 1}. ${route.from} → ${route.to}: ${zoneType} ${zoneType === route.expected ? '✅' : '❌'} (expected: ${route.expected})`)
  })
}
