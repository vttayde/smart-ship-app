...
export class PricingCalculator {
  ...
  getBillableWeight(actualWeight: number, dimensions: { length: number; width: number; height: number }): number {
  ...
  calculateCourierPrice(
    ...
    const pricingRule = PRICING_RULES.find(rule => 
    ...
    const breakdown: { description: string; amount: number; type: 'base' | 'additional' | 'tax' | 'discount' }[] = []
    ...
    if (billableWeight > weightSlab.minWeight) {
      ...
      breakdown.push({ description: `Additional Weight (${extraWeight}kg)`, amount: additionalWeightCharges, type: 'additional' })
    }
    ...
    const subtotal = basePrice + additionalWeightCharges + fuelSurcharge + codCharges - discountAmount
  ...
  private isDiscountApplicable(discount: Discount, request: ShippingRequest, billableWeight: number): boolean {
  ...
  getDeliveryEstimate(courierId: string, serviceTypeId: string, fromCity: string, toCity: string) {
    ...
    const timeframe = DELIVERY_TIMEFRAMES.find(tf => 
  ...
  generateQuotes(request: ShippingRequest): CourierQuote[] {
    ...
    for (const courier of COURIER_SERVICES) {
      ...
      try {
        ...
        const delivery = this.getDeliveryEstimate(courier.id, serviceTypeId, request.fromCity, request.toCity)
        ...
        const quote: CourierQuote = {
          ...
          pricing: {
            ...
            discounts: pricing.breakdown
              ...
              .map(item => ({ name: item.description, amount: Math.abs(item.amount), type: item.type })),
            taxes: pricing.breakdown
              .filter(item => item.type === 'tax')
              .map(item => ({ name: item.description, amount: item.amount, percentage: this.GST_RATE })),
