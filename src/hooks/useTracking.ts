...
export const useTracking = () => {
  ...
  const clearTracking = useCallback(() => dispatch(clearCurrentTracking()), [dispatch]);
  ...
  const clearHistory = useCallback(() => dispatch(clearSearchHistory()), [dispatch]);

  const clearError = useCallback(() => dispatch(clearTrackingError()), [dispatch]);

  const markAsRefreshed = useCallback(() => dispatch(updateLastRefresh()), [dispatch]);

  // Utility functions
  const getTrackingFromHistory = useCallback(
    (trackingNumber: string) => {
      return trackingHistory.find(item => item.shipment.trackingNumber === trackingNumber);
    },
    [trackingHistory]
  );

  const isTrackingNumber = useCallback((value: string) => {
    // Basic validation for tracking number format
    return /^[A-Z0-9]{6,20}$/i.test(value.trim());
  }, []);

  const formatTrackingNumber = useCallback((value: string) => {
