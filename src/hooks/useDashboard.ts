...
export const useDashboard = () => {
  ...
  const loadAlerts = useCallback((limit: number = 5) => dispatch(fetchAlerts(limit)), [dispatch]);
  ...
  const removeAlert = useCallback((alertId: string) => dispatch(dismissAlert(alertId)), [dispatch]);
